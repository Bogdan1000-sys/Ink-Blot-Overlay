...
def RunWSubprocess(key, args={}):
    args["selectionWindow"] = selectionWindow.objectName()
    uiApp.subprocessing.runProcess(key, f"Widgets/{key}/main.py", args)
    ...
    def UpdatePreviewInfo(self, useSound=True):
        ...
        def RunSelection():
            for widget in selectedWidgets:
                RunWSubprocess(widget)

        def RunCurrent():
            RunWSubprocess(self.selectedWidget)

        def TerminateActive():
            uiApp.subprocessing.terminateProcesses()

        def TerimnateCurrent():
            uiApp.subprocessing.removeProcess(self.selectedWidget)
                    ...
                    def onRun():
                        ...
                        def load():
                            ...
                            for i, widget in enumerate(UserSettings["General"]["activeWidgets"]):
                                QTimer.singleShot(GetQTime(0.25) * i, lambda w=widget: RunWSubprocess(w))
