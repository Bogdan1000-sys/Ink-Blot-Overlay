...
from PyQt6.QtGui import QWheelEvent, QPixmap, QVector3D
import sys, math, json, random, subprocess, threading, os
...
# -- Functions --
from Functions import GetUserSettings, applyStyleClass, initSound, playSound, RegisterAdaptableText, AppendUserSettings, GetQTime, CloneAnimation, PrintText, RunWidgetProcess
...
def RunWProcess(path=None, key=None):
    if path == None or key == None or root == None or uiApp == None: return
    RunWidgetProcess(path, key, root, uiApp)
...
class WidgetSelectingWindow(InteractableWindow):
    ...
    def UpdatePreviewInfo(self, useSound=True):
        ...
        selectedWidgets = UserSettings.get("selectedWidgets", [])
        ...
        def RunSelection():
            for widget in selectedWidgets:
                RunWProcess(f"Widgets/{widget}/main.py", widget)

        def RunCurrent():
            RunWProcess(f"Widgets/{self.selectedWidget}/main.py", self.selectedWidget)
        ...
        def OnClick(button: str):
            ...
            AppendUserSettings({"selectedWidgets": selectedWidgets, "activeWidgets": activeWidgets})
    ...
    def __init__(self, **kwargs):
        ...
        def startShowing():
            ...
            def printOptions():
                ...
                def optionBasic():
                    playSound(self.SFX["optionsClickSound"])
                ...
                class OptionsModule():
                    ...
                    def onStart():
                        ...
                        AppendUserSettings({"selectedWidgets": [], "activeWidgets": []})
                        self.Hide(onFinished=createSettingWindow, Hard=True)  
                    ...
                    def onRun():
                        ...
                        def load():
                            createSettingWindow()
                            for i, widget in enumerate(UserSettings["activeWidgets"]):
                                QTimer.singleShot(GetQTime(0.25) * i, lambda w=widget: RunWProcess(f"Widgets/{w}/main.py", w))
                    ...
                    def onOpen():
                        ...
                        if len(UserSettings["activeWidgets"]) > 0:
                            AppendUserSettings({"selectedWidgets": UserSettings["activeWidgets"], "activeWidgets": []})
                ...
                def newPack():
                    UpdateUserSettings()
                    global UserSettings, settingWindow

                    if len(UserSettings["activeWidgets"]) > 0 or len(UserSettings["selectedWidgets"]) > 0:
                ...
                if len(UserSettings["selectedWidgets"]) > 0 or len(UserSettings["activeWidgets"]) > 0:
...
if __name__ == "__main__":
    welcomeWindow = WelcomeWindow(Size={"width": 1000, "height": 600}, titleKey="titles/welcome")
    # settingWindow = WidgetSelectingWindow(titleKey="titles/widgetSelection")
