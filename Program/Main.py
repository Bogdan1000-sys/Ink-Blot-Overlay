...
# -- Variables --
generalSettingsIsActive = False
generalSettingsDebounce = False
...
def RunWMultiprocess(key, args=(), joinProcess=False):
    if key == None: return
    uiApp.multiprocessing.runProcess(key=key, function=Widgets[key], arguments=args, joinProcess=joinProcess)
    widgetsMenuWindow.UpdateListeners()

def EndWMultiprocess(key):
    if key == None: return
    uiApp.multiprocessing.removeProcess(key)
    widgetsMenuWindow.UpdateListeners()

def TerminateWMultiprocesses():
    uiApp.multiprocessing.terminateProcesses()
    widgetsMenuWindow.UpdateListeners()
...
class WidgetsMenuWindow(InteractableWindow):
    ...
    def onMessage(self, msg: str):
        ...
        def CreateCover():
            if self.Cover != None: return
            self.Cover = QFrame(parent=self)
            self.Cover.setProperty("class", "shadeCover")
            self.Cover.setStyleSheet("""
                QFrame[class='shadeCover'] {
        ...
        self.codeFunctions = {
            "InitializeCode": lambda: print("Code Initialized"),

            "UpdatePreview": lambda: self.UpdatePreviewInfo(False),
            "CreateCover": lambda: CreateCover(),
            "DestroyCover": lambda: DestroyCover()
        }
    ...
    def UpdateListeners(self):
        for listener in self.connectionListeners.values():
            listener.stop()
            listener.deleteLater()
        self.connectionListeners.clear()

        for key, connection in uiApp.mpConnections.items():
            listener = ConnectionListener(connection)
            listener.messageReceived.connect(self.onMessage)
            listener.start()

            self.connectionListeners[key] = listener
    ...
    def UpdatePreviewInfo(self, useSound=True):
        ...
        def RunSelection():
            for widget in selectedWidgets:
                RunWMultiprocess(widget)

        def RunCurrent():
            RunWMultiprocess(self.selectedWidget)

        def TerminateActive():
            for connection in uiApp.mpConnections.values():
                try: connection.send("code:Close")
                except Exception:pass
            QTimer.singleShot(GetQTime(0.75), lambda: TerminateWMultiprocesses())

        def TerimnateCurrent():
            if uiApp.mpConnections.get(self.selectedWidget):
                try: uiApp.mpConnections[self.selectedWidget].send("code:Close")
                except Exception: pass
            QTimer.singleShot(GetQTime(0.75), lambda: EndWMultiprocess(self.selectedWidget))
    ...
    def __init__(self, **kwargs):
        ...
        for info in self.WidgetButtons.values():
            name = info["data"]["Key"]
            info["button"].clicked.connect( makeButtonClickHandler(name) )

        self.onMessage("code:InitializeCode")
        self.UpdatePreviewInfo(useSound=False)

        # GENERAL SETTINGS ----------------------------------

        def Manipulate_GeneralSettings():
            global generalSettingsIsActive, generalSettingsDebounce

            if generalSettingsDebounce: return
            generalSettingsDebounce = True
            def changeDebounce():
                global generalSettingsDebounce
                generalSettingsDebounce = False
            QTimer.singleShot(GetQTime(0.5), changeDebounce)

            if uiApp.mpConnections.get("GeneralSettings") == None:
                generalSettingsIsActive = True
                RunWMultiprocess("GeneralSettings")
            else:
                generalSettingsIsActive = False
                uiApp.mpConnections["GeneralSettings"].send(f"code:Close")

        self.buttons["generalSettingsButton"].clicked.connect( Manipulate_GeneralSettings )
